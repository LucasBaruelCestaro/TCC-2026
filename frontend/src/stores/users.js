  ...
  getters: {
    ...
    professores: (state) => {
      return state.users.filter(
        (u) => u.role === "Professor" && u.ativo === true,
      );
    },

    processoPedagogico: (state) => {
      return state.users.filter(
        (u) => u.role === "Processo pedagógico" && u.ativo === true,
      );
    },
    ...
    getByRegistro: (state) => (registro) => {
      ...
      return state.users.filter(
        (u) =>
          u.registro &&
          u.registro.toString().includes(registro.toString()) &&
          u.ativo === true,
      );
    },
  ...
  actions: {
    ...
    saveToLocalStorage() {
      try {
        ...
        console.log("Usuários salvos no localStorage:", this.users); // Debug
      } catch (error) {
    ...
    createUser(userData) {
      ...
      const emailRegex =
        /^[a-zA-Z0-9][a-zA-Z0-9._%+-]{0,63}@[a-zA-Z0-9][a-zA-Z0-9.-]+\.[a-zA-Z]{2,}$/;
      ...
      if (
        !userData.role ||
        !["Professor", "Processo pedagógico"].includes(userData.role)
      ) {
      ...
      let senhaTemporaria = userData.senha;
      if (!senhaTemporaria && userData.dataNascimento) {
        // Converte de AAAA-MM-DD para DDMMAAAA
        const partes = userData.dataNascimento.split("-");
        if (partes.length === 3) {
          senhaTemporaria = `${partes[2]}${partes[1]}${partes[0]}`;
        }
      }
      if (!senhaTemporaria) {
      ...
      console.log("Usuário criado com sucesso:", newUser); // Debug
      console.log("Total de usuários:", this.users.length); // Debug
    ...
    updateUser(id, updates) {
      ...
      if (
        updates.disciplina !== undefined &&
        this.users[index].role === "Professor"
      ) {
    ...
    isPrimeiroAcesso(registro) {
      const user = this.users.find(
        (u) => u.registro === registro && u.ativo === true,
      );
